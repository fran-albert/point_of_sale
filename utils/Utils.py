...
def create_main_window_menu(parent):
    ...
    vendedores_action = QAction(QIcon(Utils.get_full_path("utils/img/icons8-grupo-de-usuarios-hombre-y-mujer-48.png")), "Vendedores", parent)
    menu_menu.addAction(vendedores_action)

    categorias_action = QAction(QIcon(Utils.get_full_path("utils/img/icons8-almacén-48.png")), "Categorias", parent)
    menu_menu.addAction(categorias_action)

    productos_action = QAction(QIcon(Utils.get_full_path("utils/img/icons8-almacén-48.png")), "Productos", parent)
    menu_menu.addAction(productos_action)

    proveedores_action = QAction(QIcon(Utils.get_full_path("utils/img/icons8-proveedor-48.png")), "Proveedores", parent)
    menu_menu.addAction(proveedores_action)

    ordenes_action = QAction(QIcon(Utils.get_full_path("utils/img/icons8-recibo-48.png")), "Órdenes de Compra", parent)
    menu_menu.addAction(ordenes_action)

    movimientos_menu = QMenu("Movimientos", parent)
    menu_bar.addMenu(movimientos_menu)

    ventas_action = QAction(QIcon(Utils.get_full_path("utils/img/icons8-caja-registradora-48.png")), "Ventas", parent)
    movimientos_menu.addAction(ventas_action)

    reportes_menu = QMenu("Reportes", parent)
    menu_bar.addMenu(reportes_menu)

    reporte_stock_minimo_action = QAction(QIcon(Utils.get_full_path("utils/img/icons8-pdf-48.png")), "Reporte de Stock Mínimo", parent)
    reportes_menu.addAction(reporte_stock_minimo_action)

    reporte_stock_action = QAction(QIcon(Utils.get_full_path("utils/img/icons8-pdf-48.png")), "Reporte de Stock", parent)
    reportes_menu.addAction(reporte_stock_action)

    reporte_ventas_action = QAction(QIcon(Utils.get_full_path("utils/img/icons8-pdf-48.png")), "Reporte de Ventas", parent)
...
def init_header(parent, width, nombre_usuario, menu_bar_height):
    ...
    pixmap = QPixmap(Utils.get_full_path("utils/img/logosinfondo.png")).scaled(300, 250, Qt.KeepAspectRatio)
